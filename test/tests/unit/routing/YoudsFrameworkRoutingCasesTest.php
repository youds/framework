...
class SampleRouting extends YoudsFrameworkRouting
{
	...
	{
		$this->importRoutes(unserialize(file_get_contents(YoudsFrameworkConfigCache::checkConfig($cfg, $ctx))));
	}
}

class BaseTestCallback extends YoudsFrameworkRoutingCallback
{
...
class TestCallbackLAN extends YoudsFrameworkRoutingCallback
{
	public function onGenerate(array $defaultParameters, array &$userParameters, array &$userOptions)
	{
...
class TestCallbackRAN extends YoudsFrameworkRoutingCallback
{
...
class YoudsFrameworkRoutingCasesTest extends YoudsFrameworkUnitTestCase
{
	protected $_r = null;
	...
	{
		$this->_config = YoudsFrameworkConfig::get('core.config_dir') . '/tests/' . $config;
	...
	protected function doTestMatch($input, $routes, $parameters, $message = null)
	{
		...
		$this->assertEquals($routes, $rq->getAttribute('matched_routes', 'org.youds.routing'), $message);
	...
	{
		$r = $this->_r;
		$r->loadTestConfig(YoudsFrameworkConfig::get('core.config_dir') . '/tests/routing_simple.xml', 'test1');
	...
	public function testErrors()
	{
		...
		try {
			$r->loadTestConfig(YoudsFrameworkConfig::get('core.config_dir') . '/tests/routing_errors.xml', 'SameNameDirectChild');
			$this->fail('Expected YoudsFrameworkException not thrown for declaring direct childs with the same name!');
		} catch(YoudsFrameworkException $e) {
		}

		try {
			$r->loadTestConfig(YoudsFrameworkConfig::get('core.config_dir') . '/tests/routing_errors.xml', 'SameNameIndirectChild');
			$this->fail('Expected YoudsFrameworkException not thrown for declaring indirect childs with the same name!');
		} catch(YoudsFrameworkException $e) {
		}

		try {
			$r->loadTestConfig(YoudsFrameworkConfig::get('core.config_dir') . '/tests/routing_errors.xml', 'SameNameInOverwrittenHierarchy');
			$this->fail('Expected YoudsFrameworkException not thrown for declaring childs with the same name when inserting a new child hierarchy on overwriting a pattern!');
		} catch(YoudsFrameworkException $e) {
		}
	...
	public function testHttpRedirectCallback()
	{
		...
		$rp = $r->execute(); /** @var $rp YoudsFrameworkWebResponse */

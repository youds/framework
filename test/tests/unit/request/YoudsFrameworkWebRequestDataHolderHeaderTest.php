...
class YoudsFrameworkWebRequestDataHolderHeaderTest extends YoudsFrameworkWebRequestDataHolderTest
{
	/**
	...
	public function testMergeHeaders()
	{
		...
		$dh2 = new YoudsFrameworkWebRequestDataHolder(
			array(
				YoudsFrameworkWebRequestDataHolder::SOURCE_HEADERS => $addHeaders,
			)
		);
	...
	public function testMergeHeaders2()
	{
		...
		$this->assertEquals(
			$expected,
			$dh->getHeaders(),
			'Failed asserting that headers from a dataholder not implementing YoudsFrameworkIHeadersRequestDataHolder are not merged.'
		);
	}

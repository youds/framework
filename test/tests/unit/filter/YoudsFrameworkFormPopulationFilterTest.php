...
class YoudsFrameworkFormPopulationFilterTest extends YoudsFrameworkUnitTestCase
{
	
	/**
	 * @var YoudsFrameworkContext
	 */
	private $_context;
	...
	{
		$this->_context = YoudsFrameworkContext::getInstance('test');
	}
	...
	public function testFieldErrorMessage()
	{
		...
		$vm = $this->_context->createInstanceFor('validation_manager'); /** @var $vm \YoudsFrameworkValidationManager */
	 ...
	 * @param \YoudsFrameworkRequestDataHolder|array $parameters
	 * @param \YoudsFrameworkValidationManager $validationManager
	 * @param array $config
	 * @return string
	 */
	protected function executeFormPopulationFilter($content, $parameters, $validationManager = null, array $config = array())
	{
		...
		if($parameters instanceof YoudsFrameworkRequestDataHolder) {
			$rd = $parameters;
		} else {
			$rd = new YoudsFrameworkRequestDataHolder(array(
				YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => $parameters,
			));
		}
		...
		$fpf = new YoudsFrameworkFormPopulationFilter();
		...
		$filterChain = new YoudsFrameworkFilterChain();

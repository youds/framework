...
class YoudsFrameworkDateFormatTest extends YoudsFrameworkUnitTestCase
{

	public function testParse()
	{
		$locale = $this->getContext()->getTranslationManager()->getLocale('@timezone=GMT+0200');
		$inputFormat = new YoudsFrameworkDateFormat('yyyy-MM-dd HH:mm:ss');
		...
		$this->assertTrue(($tz instanceof YoudsFrameworkSimpleTimezone), 'Failed asserting that the created tz is an YoudsFrameworkSimpleTimezone.');
	...
	public function testParseWithSystemDefaultTimeZone()
	{
		...
		$currentLocale = YoudsFrameworkLocale::parseLocaleIdentifier($tm->getCurrentLocale()->getIdentifier());
	...
	public function testFormatWithProvidedLocaleTimeZone()
	{
		...
		$this->assertEquals('2008-11-20 04:00:00 ' . $zoneId, $inputFormat->format($cal, YoudsFrameworkCalendar::GREGORIAN, '@timezone=' . $zoneId));
	}

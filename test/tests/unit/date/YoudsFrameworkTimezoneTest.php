 ...
 */
class YoudsFrameworkTimeZoneTest extends BaseCalendarTest
{
...
// ---------------------------------------------------------------------------------
	...
	{
		// TODO: this method needs a general overhaul to fit youds

		$id = "NewGMT";
		$offset = 12345;

		$zone = new YoudsFrameworkSimpleTimeZone($this->tm, $offset, $id);
...
/*
		YoudsFrameworkTimeZone::adoptDefault($zone);
		$defaultzone = YoudsFrameworkTimeZone::createDefault();
		$this->assertFalse($defaultzone == $zone || !($defaultzone->__is_equal($zone)));
		YoudsFrameworkTimeZone::adoptDefault($saveDefault);
*/
	}
 ...
 */
	public function TestRuleAPI()
	{
		...
		$zone = new YoudsFrameworkSimpleTimeZone($this->tm, $offset, "TestZone");
		$this->assertFalse($zone->useDaylightTime());
		...
		$gc->clear();
		$gc->set(1990, YoudsFrameworkDateDefinitions::MARCH, 1);
		$marchOneStd = $gc->getTime(); // Local Std time midnight
		$gc->clear();
		$gc->set(1990, YoudsFrameworkDateDefinitions::JULY, 1);
		...
		$zone->setStartRule(YoudsFrameworkDateDefinitions::MARCH, 1, 0, $startHour);
		$zone->setEndRule  (YoudsFrameworkDateDefinitions::JULY,  1, 0, $endHour);
		...
		$this->myTestUsingBinarySearch($zone, $this->date(90, YoudsFrameworkDateDefinitions::JANUARY, 1), $this->date(90, YoudsFrameworkDateDefinitions::JUNE, 15), $marchOne);
		$this->myTestUsingBinarySearch($zone, $this->date(90, YoudsFrameworkDateDefinitions::JUNE, 1), $this->date(90, YoudsFrameworkDateDefinitions::DECEMBER, 31), $julyOne);
...
// -------------------------------------
	...
	public function testVariousAPI518()
	{
		...
		$d = $this->date(97, YoudsFrameworkDateDefinitions::APRIL, 30);
		...
		$gc->setTime($d);
		$this->assertEquals(floatval(-7 * self::millisPerHour), $time_zone->getOffset(YoudsFrameworkGregorianCalendar::AD, $gc->get(YoudsFrameworkDateDefinitions::YEAR), $gc->get(YoudsFrameworkDateDefinitions::MONTH), $gc->get(YoudsFrameworkDateDefinitions::DATE), $gc->get(YoudsFrameworkDateDefinitions::DAY_OF_WEEK), 0));
	}
...
// -------------------------------------
	...
	public function testGetAvailableIDs913()
	{
		...
		*buf += "TimeZone::createEnumeration(GMT+01:00) = { ";
		...
		s = TimeZone::createEnumeration(1 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		s_length = s->count(ec);
		for (i = 0; i < s_length;++i) {
				if (i > 0) *buf += ", ";
				*buf += *s->snext(ec);
 ...
 */
	public function testDisplayName()
	{
		...
		$data = array(
			array(false, YoudsFrameworkTimeZone::SHORT, 'PST'),
			array(true,  YoudsFrameworkTimeZone::SHORT, 'PDT'),
			array(false, YoudsFrameworkTimeZone::LONG, 'Pacific Standard Time'),
			array(true,  YoudsFrameworkTimeZone::LONG, 'Pacific Daylight Time'),
		);
		...
		$zone2 = new YoudsFrameworkSimpleTimeZone($this->tm, 0, "America/Los_Angeles");

		$zone2->setStartRule(YoudsFrameworkDateDefinitions::JANUARY, 1, 0, 0);
		$zone2->setEndRule(YoudsFrameworkDateDefinitions::DECEMBER, 31, 0, 0);
...
//	************************************************************
		...
		$zone2 = new YoudsFrameworkSimpleTimeZone($this->tm, 90*60*1000, "xyzzy");
		$name = $zone2->getDisplayName($enLocale);
 ...
 */
	public function testDSTSavings()
	{
		...
		$tz = new YoudsFrameworkSimpleTimeZone($this->tm, -5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, "dstSavingsTest",
																		YoudsFrameworkDateDefinitions::MARCH, 1, 0, 0, YoudsFrameworkDateDefinitions::SEPTEMBER, 1, 0, 0,
																		(0.5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR));

		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $tz->getRawOffset());

		$this->assertTrue($tz->useDaylightTime(), 'Test time zone should use DST but claims it doesn\'t.');
		
		$this->assertEquals(0.5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $tz->getDSTSavings());

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::JANUARY, 1, 
															YoudsFrameworkDateDefinitions::THURSDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::JUNE, 1, YoudsFrameworkDateDefinitions::MONDAY,
															10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-4.5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		$tz->setDSTSavings(YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::JANUARY, 1,
															YoudsFrameworkDateDefinitions::THURSDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);
		...
		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::JUNE, 1, YoudsFrameworkDateDefinitions::MONDAY,
															10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-4 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);
	}
 ...
 */
	public function testAlternateRules()
	{
		...
		$tz = new YoudsFrameworkSimpleTimeZone($this->tm, -5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, "alternateRuleTest");

		// test the day-of-month API
		$tz->setStartRule(YoudsFrameworkDateDefinitions::MARCH, 10, 12 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$tz->setEndRule(YoudsFrameworkDateDefinitions::OCTOBER, 20, 12 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::MARCH, 5,
															YoudsFrameworkDateDefinitions::THURSDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::MARCH, 15,
															YoudsFrameworkDateDefinitions::SUNDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-4 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::OCTOBER, 15,
															YoudsFrameworkDateDefinitions::THURSDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-4 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::OCTOBER, 25,
															YoudsFrameworkDateDefinitions::SUNDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		// test the day-of-week-after-day-in-month API
		$tz->setStartRule(YoudsFrameworkDateDefinitions::MARCH, 10, YoudsFrameworkDateDefinitions::FRIDAY, intval(12 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR), true);
		$tz->setEndRule(YoudsFrameworkDateDefinitions::OCTOBER, 20, YoudsFrameworkDateDefinitions::FRIDAY, intval(12 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR), false);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::MARCH, 11,
															YoudsFrameworkDateDefinitions::WEDNESDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);

		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::MARCH, 14,
															YoudsFrameworkDateDefinitions::SATURDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-4 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);
		...
		$offset = $tz->getOffset(YoudsFrameworkGregorianCalendar::AD, 1998, YoudsFrameworkDateDefinitions::OCTOBER, 17,
															YoudsFrameworkDateDefinitions::SATURDAY, 10 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR);
		$this->assertEquals(-5 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR, $offset);
	}
	...
	{
		$H = YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR;
		...
		foreach($data as $item) {
			...
			$raw = 0;
			$dst = 0;
			$when = $item['time'] * YoudsFrameworkDateDefinitions::MILLIS_PER_SECOND;
	...
	public function testFebruary() {
		...
		$tz1 = new YoudsFrameworkSimpleTimeZone($this->tm, -3 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR,          // raw offset: 3h before (west of) GMT
		                               "nov-feb",
		                               YoudsFrameworkDateDefinitions::NOVEMBER, 1, YoudsFrameworkDateDefinitions::SUNDAY,   // start: November, first, Sunday
		                               0,                               //        midnight wall time
		                               YoudsFrameworkDateDefinitions::FEBRUARY, -1, YoudsFrameworkDateDefinitions::SUNDAY,  // end:   February, last, Sunday
		                               0                                //        midnight wall time
		);
		...
		$tz2 = new YoudsFrameworkSimpleTimeZone($this->tm, -3 * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR,          // raw offset: 3h before (west of) GMT 
		                               "nov-feb2", 
		                               YoudsFrameworkDateDefinitions::NOVEMBER, 1, -YoudsFrameworkDateDefinitions::SUNDAY,  // start: November, 1 or after, Sunday 
		                               0,                               //        midnight wall time 
		                               YoudsFrameworkDateDefinitions::FEBRUARY, -29, -YoudsFrameworkDateDefinitions::SUNDAY,// end:   February, 29 or before, Sunday 
		                               0                                //        midnight wall time 
		); 

		// Gregorian calendar with the UTC time zone for getting sample test date/times.
		$gc = new YoudsFrameworkGregorianCalendar(YoudsFrameworkTimeZone::getGMT($this->tm));

		$data = array(
			array( 'year' => 2006, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  5, 'hour' => 02, 'minute' => 59, 'second' => 59, 'offsetHours' => -3 ),
			array( 'year' => 2006, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  5, 'hour' => 03, 'minute' => 00, 'second' => 00, 'offsetHours' => -2 ),
			array( 'year' => 2007, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 25, 'hour' => 01, 'minute' => 59, 'second' => 59, 'offsetHours' => -2 ),
			array( 'year' => 2007, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 25, 'hour' => 02, 'minute' => 00, 'second' => 00, 'offsetHours' => -3 ),

			array( 'year' => 2007, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  4, 'hour' => 02, 'minute' => 59, 'second' => 59, 'offsetHours' => -3 ),
			array( 'year' => 2007, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  4, 'hour' => 03, 'minute' => 00, 'second' => 00, 'offsetHours' => -2 ),
			array( 'year' => 2008, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 24, 'hour' => 01, 'minute' => 59, 'second' => 59, 'offsetHours' => -2 ),
			array( 'year' => 2008, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 24, 'hour' => 02, 'minute' => 00, 'second' => 00, 'offsetHours' => -3 ),

			array( 'year' => 2008, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  2, 'hour' => 02, 'minute' => 59, 'second' => 59, 'offsetHours' => -3 ),
			array( 'year' => 2008, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  2, 'hour' => 03, 'minute' => 00, 'second' => 00, 'offsetHours' => -2 ),
			array( 'year' => 2009, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 22, 'hour' => 01, 'minute' => 59, 'second' => 59, 'offsetHours' => -2 ),
			array( 'year' => 2009, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 22, 'hour' => 02, 'minute' => 00, 'second' => 00, 'offsetHours' => -3 ),

			array( 'year' => 2009, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  1, 'hour' => 02, 'minute' => 59, 'second' => 59, 'offsetHours' => -3 ),
			array( 'year' => 2009, 'month' => YoudsFrameworkDateDefinitions::NOVEMBER, 'day' =>  1, 'hour' => 03, 'minute' => 00, 'second' => 00, 'offsetHours' => -2 ),
			array( 'year' => 2010, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 28, 'hour' => 01, 'minute' => 59, 'second' => 59, 'offsetHours' => -2 ),
			array( 'year' => 2010, 'month' => YoudsFrameworkDateDefinitions::FEBRUARY, 'day' => 28, 'hour' => 02, 'minute' => 00, 'second' => 00, 'offsetHours' => -3 ),
		);
...
#	    int32_t t, i, raw, dst;
		...
		for($t = 0; $t < count($timezones); ++$t) {
			...
			for($i = 0; $i < count($data); ++$i) {
				...
				if(($raw + $dst) != $data[$i]['offsetHours'] * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR) {
					$this->fail(sprintf("test case %d.%d: tz.getOffset(%04d-%02d-%02d %02d:%02d:%02d) returns %d+%d != %d",
					                    $t, $i,
					                    $data[$i]['year'], $data[$i]['month'] + 1, $data[$i]['day'],
					                    $data[$i]['hour'], $data[$i]['minute'], $data[$i]['second'],
					                    $raw, $dst, $data[$i]['offsetHours'] * YoudsFrameworkDateDefinitions::MILLIS_PER_HOUR)
					);
				}
			}
		}

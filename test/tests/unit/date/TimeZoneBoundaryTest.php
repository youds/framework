...
// -------------------------------------
	...
	protected function verifyDST($d, $time_zone, $expUseDaylightTime, $expInDaylightTime, $expZoneOffset, $expDSTOffset)
	{
		...
		$gc->setTime($d);
		$offset = $time_zone->getOffset($gc->get(YoudsFrameworkDateDefinitions::ERA),
				$gc->get(YoudsFrameworkDateDefinitions::YEAR), $gc->get(YoudsFrameworkDateDefinitions::MONTH),
				$gc->get(YoudsFrameworkDateDefinitions::DATE), $gc->get(YoudsFrameworkDateDefinitions::DAY_OF_WEEK),
				(($gc->get(YoudsFrameworkDateDefinitions::HOUR_OF_DAY) * 60 + $gc->get(YoudsFrameworkDateDefinitions::MINUTE)) * 60 + $gc->get(YoudsFrameworkDateDefinitions::SECOND)) * 1000 + $gc->get(YoudsFrameworkDateDefinitions::MILLISECOND)
		);
...
// -------------------------------------
	...
	protected function verifyMapping(&$cal, $year, $month, $dom, $hour, $epochHours)
	{
		...
		$cal->setTime($ed);
		$this->assertEquals($year, $cal->get(YoudsFrameworkDateDefinitions::YEAR));
		$this->assertEquals($month, $cal->get(YoudsFrameworkDateDefinitions::MONTH));
		$this->assertEquals($dom, $cal->get(YoudsFrameworkDateDefinitions::DATE));
		$this->assertEquals(floatval($hour * 3600000), $cal->get(YoudsFrameworkDateDefinitions::MILLISECONDS_IN_DAY));
}
	...
	public function testBoundaries()
	{
		...
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 3,  0, 238904.0);
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 4,  0, 238928.0);
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 5,  0, 238952.0);
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 5, 23, 238975.0);
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 6,  0, 238976.0);
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 6,  1, 238977.0);
		$this->verifyMapping($tempcal, 1997, YoudsFrameworkDateDefinitions::APRIL, 6,  3, 238978.0);

		$utc = $this->tm->createTimeZone("UTC");
		$utccal =  $this->tm->createCalendar($utc);
		$this->verifyMapping($utccal, 1997, YoudsFrameworkDateDefinitions::APRIL, 6, 0, 238968.0);

		$save = $this->tm->getDefaultTimeZone();
		//YoudsFrameworkTimeZone::setDefault($pst);
		$this->tm->setDefaultTimeZone($pst->getId());
		
		// DST changeover for PST is 4/6/1997 at 2 hours past midnight
		// at 238978.0 epoch hours.
		$tempcal->clear();
		$tempcal->set(1997, YoudsFrameworkDateDefinitions::APRIL, 6);
		$d = $tempcal->getTime();
		...
		//YoudsFrameworkTimeZone::setDefault($save);
		$this->tm->setDefaultTimeZone($save->getId());


			$d = $this->date(97, YoudsFrameworkDateDefinitions::APRIL, 6);
...
//			YoudsFrameworkTimeZone::setDefault($tz = $this->tm->createTimeZone("America/Los_Angeles"));
...
// -------------------------------------
	...
	{
		$tz = new YoudsFrameworkSimpleTimeZone($this->tm, -8 * $this->ONE_HOUR, "Test_1", YoudsFrameworkDateDefinitions::AUGUST, 2, YoudsFrameworkDateDefinitions::TUESDAY, 2 * $this->ONE_HOUR, YoudsFrameworkDateDefinitions::MARCH, 15, 0, 2 * $this->ONE_HOUR);
		...
		$tz = new YoudsFrameworkSimpleTimeZone($this->tm -8 * $this->ONE_HOUR, "Test_2", YoudsFrameworkDateDefinitions::APRIL, 14, - YoudsFrameworkDateDefinitions::WEDNESDAY, 2 * $this->ONE_HOUR, YoudsFrameworkDateDefinitions::SEPTEMBER, -20, - YoudsFrameworkDateDefinitions::SUNDAY, 2 * $this->ONE_HOUR);
...
// -------------------------------------
	...
	{
		$d = $this->date($year - 1900, YoudsFrameworkDateDefinitions::JANUARY, 1);
		$time = $d;

...
class YoudsFrameworkNumberValidatorTest extends YoudsFrameworkUnitTestCase
{

	/**
	 * @var YoudsFrameworkValidationManager
	 */
	protected $vm;
	...
	{
		$number = '1.23';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('invalid argument'), $parameters = array('type' => 'int'));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::ERROR, $result);
	...
	{
		$number = '1.23';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('invalid argument'), $parameters = array('type' => 'float'));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result);
		$this->assertEquals($number, $rd->getParameter('number'));
	...
	{
		$number = '1.23';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('invalid argument'), $parameters = array('type' => 'float', 'cast_to' => 'int'));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result);
	...
	{
		$number = '1';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('invalid argument'), $parameters = array('type' => 'float', 'cast_to' => 'float'));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result);
		$this->assertEquals(1, $rd->getParameter('number'));
	...
	public function testMinFail()
	{
		...
		$number = '1';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('min' => $minError), $parameters = array('type' => 'int', 'min' => 2));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::ERROR, $result);
	...
	public function testMinSuccess()
	{
		$minError = 'value too low';
		$number = '1';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('min' => $minError), $parameters = array('type' => 'int', 'min' => 1));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result);
	...
	public function testMaxFail()
	{
		...
		$number = '2';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('max' => $maxError), $parameters = array('type' => 'int', 'max' => 1));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::ERROR, $result);
	...
	public function testMaxSuccess()
	{
		$maxError = 'value too high';
		$number = '2';
		$validator = $this->vm->createValidator('YoudsFrameworkNumberValidator', array('number'), array('max' => $maxError), $parameters = array('type' => 'int', 'max' => 2));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('number' => $number)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result);

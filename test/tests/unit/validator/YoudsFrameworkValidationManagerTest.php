...
class MyValidationManager extends YoudsFrameworkValidationManager
{
	public function getChildren() { return $this->children; }
}

class YoudsFrameworkValidationManagerTest extends YoudsFrameworkUnitTestCase 
{
	private $_vm = null;
	...
	{
		$this->assertTrue($this->_vm->getDependencyManager() instanceof YoudsFrameworkDependencyManager);
	}
	
	public function testgetBase()
	{
		$this->_vm->removeParameter('base');
		$this->assertEquals($this->_vm->getBase(), new YoudsFrameworkVirtualArrayPath(''));
		$this->_vm->setParameter('base', '');
		$this->assertEquals($this->_vm->getBase(), new YoudsFrameworkVirtualArrayPath(''));
		$this->_vm->setParameter('base', 'foo[bar]');
		$this->assertEquals($this->_vm->getBase(), new YoudsFrameworkVirtualArrayPath('foo[bar]'));
	}
	...
	public function testExecute()
	{
		...
		$this->assertTrue($this->_vm->execute(new YoudsFrameworkRequestDataHolder()));
		...
		$this->_vm->registerValidators(array($val1, $val2));
		$this->assertFalse($this->_vm->execute(new YoudsFrameworkRequestDataHolder()));
		$this->assertTrue($val1->validated);
		$this->assertTrue($val2->validated);
		$this->_vm->clear();
		$val1->clear();
	...
	{
		$this->assertEquals(YoudsFrameworkValidator::NOT_PROCESSED, $this->_vm->getResult());
	}
	...
	public function testTransfersDependTokens()
	{
		...
		$vm->execute(new YoudsFrameworkRequestDataHolder());

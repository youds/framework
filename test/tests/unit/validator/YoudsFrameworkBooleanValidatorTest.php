...
class YoudsFrameworkBooleanValidatorTest extends YoudsFrameworkUnitTestCase
{
	...
	{
		$validator = $this->vm->createValidator('YoudsFrameworkBooleanValidator', array('bool'), array('invalid argument'), array());
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('bool' => $value)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result, 'Failed asserting that the validation succeeded.');
	...
	{
		$validator = $this->vm->createValidator('YoudsFrameworkBooleanValidator', array('bool'), array('invalid argument'), array('export' => 'exported'));
		$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('bool' => $value)));
		$result = $validator->execute($rd);
		$this->assertEquals(YoudsFrameworkValidator::ERROR, $result, 'Failed asserting that the validation failed.');
	...
	public function testDontCastOnExport() {
		...
		foreach($testValues as $value) {
			$validator = $this->vm->createValidator('YoudsFrameworkBooleanValidator', array('bool'), array('invalid argument'), array('export' => 'exported'));
			$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('bool' => $value['original'])));
			$result = $validator->execute($rd);
			$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result, 'Failed asserting that the validation succeeded.');
	...
	public function testCastOnMissingExport() {
		...
		foreach($testValues as $value) {
			$validator = $this->vm->createValidator('YoudsFrameworkBooleanValidator', array('bool'), array('invalid argument'));
			$rd = new YoudsFrameworkRequestDataHolder(array(YoudsFrameworkRequestDataHolder::SOURCE_PARAMETERS => array('bool' => $value['original'])));
			$result = $validator->execute($rd);
			$this->assertEquals(YoudsFrameworkValidator::SUCCESS, $result, 'Failed asserting that the validation succeeded.');

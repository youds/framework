...
class YoudsFrameworkPackageTask extends Task
{
	private $baseDir;
	private $buildDir;
	...
	public function main()
	{
		...
		$version = $this->project->getProperty('youds.pear.version');
		$status = $this->project->getProperty('youds.status');
...
$longDesc = <<<EOD
YoudsFramework is a full-featured MVC-style framework for PHP5 with a strong focus on structure, code reusability and flexibility.
EOD;
		...
		$p2->addInstallAs('bin/youds.bat-dist', 'youds.bat');
		$p2->addIgnoreToRelease('bin/youds-dist');
		
		// and the next release... very cool, eh? how utterly stupid is that
		$p2->addRelease();
		$p2->addInstallAs('bin/youds-dist', 'youds');
		$p2->addIgnoreToRelease('bin/youds.bat-dist');
		...
		$p2->addReplacement('bin/youds-dist', 'pear-config', '@PEAR-DIR@', 'php_dir');
		$p2->addReplacement('bin/youds-dist', 'pear-config', '@PHP-BIN@', 'php_bin');
		$p2->addReplacement('bin/youds.bat-dist', 'pear-config', '@PEAR-DIR@', 'php_dir');
		$p2->addReplacement('bin/youds.bat-dist', 'pear-config', '@PHP-BIN@', 'php_bin');

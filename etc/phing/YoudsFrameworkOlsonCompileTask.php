...
class YoudsFrameworkOlsonCompileTask extends Task
{
	private $olsonDir = '';
	private $outputDir = '';
	...
	public function main()
	{
		...
		require_once('src/youds.php');

		$this->olsonDir = realpath($this->olsonDir);
		$this->outputDir = realpath($this->outputDir);

		YoudsFrameworkConfig::set('olson.dir', $this->olsonDir);

		YoudsFrameworkConfig::set('core.app_dir', getcwd() . '/etc/olson/youds/app');
		...
		foreach($di as $file) {
			if($file->isFile()) {
				...
				if(strpos($file->getFilename(), '.') === false && $file->getFilename() != 'factory') {
					...
					$parser = new YoudsFrameworkTimeZoneDataParser();
					$parser->initialize(YoudsFrameworkContext::getInstance($context));

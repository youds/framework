...
if($_POST['step'] == count($steps)-1) {
	...
	header('Content-Disposition: attachment; filename=youds-routing-testcase-' . $_POST['mode'] . '-' . gmdate('Ymd') . '.php');
	$data = array();
...
<html>
<head>
<title>YoudsFramework Routing Test<?php if($_POST['step'] > 0): ?>, step <?php echo $_POST['step']; ?> of <?php echo (count($steps)-2); endif; ?></title>
</head>
<body>

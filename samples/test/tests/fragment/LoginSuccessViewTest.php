...
class LoginSuccessViewTest extends YoudsFrameworkViewTestCase
{
	...
	{
		$this->setArguments($this->createRequestDataHolder(array(YoudsFrameworkWebRequestDataHolder::SOURCE_PARAMETERS => array('username' => 'Chuck Norris', 'password' => 'kick'))));
		$this->getContext()->getUser()->setAttribute('redirect', 'http://www.example.com/', 'org.youds.SampleApp.login');
		$this->runView();
	...
	{
		$this->setArguments($this->createRequestDataHolder(array(YoudsFrameworkWebRequestDataHolder::SOURCE_PARAMETERS => array('username' => 'Chuck Norris', 'password' => 'kick', 'remember' => true))));
		$this->runView();
		$this->assertViewResponseHasHTTPStatus(200);
		$this->assertViewResultEquals('');
		$this->assertHasLayer('content');
